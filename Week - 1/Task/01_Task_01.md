# Class Activities — IoT Programming Introduction (Week 1)


## Activity 1 — Quick Code Write: Say Hello 

The simplest possible "write it yourself" task — no LED, no lesson notes open. Just variables, `setup()`, and Serial.

1. Declare a `String` variable holding your own name.
2. In `setup()`, start Serial at `115200` and print `"Hi, I'm <your name>!"` using your variable (not the text typed twice).
3. Upload and confirm your name shows up correctly in the Serial Monitor.


Open this Wokwi simulation to create variable:- https://wokwi.com/projects/470564412054187009

**Check yourself:**
- [ ] Program compiles and uploads
- [ ] Serial Monitor is set to `115200` baud and shows your message
- [ ] The name in the message comes from the variable, not typed directly into the print

**Completed online @ https://wokwi.com/projects/471469778793751553** 

## Activity 2 — Data Type  


On paper or in a shared doc, match each value below to its correct data type as fast as you can. Give yourself 3 minutes for all eight, then check your answers.

| Value | Type |
|-|-|
| 25 | Int |
| 22.5 | Float |
| "sensor" | String |
| true | Bool | 
| 'A' | Char |
| 255 | Byte |
| 99999 | Long |
| 3.14 | Double |

Types to choose from: `int`, `float`, `double`, `String`, `bool`, `char`, `byte`, `long`

<details>
<summary>Answer key (check after you finish)</summary>

- `25` → `int`
- `22.5` → `float`
- `"sensor"` → `String`
- `true` → `bool`
- `'A'` → `char`
- `255` → `byte` (smallest type that fits — `int` also technically works, but `byte` is the better choice)
- `99999` → `long`
- `3.14` → `double`

</details>


## Activity 3 — Variable or Constant? 

On paper or in a doc with two columns headed **Variable** and **Constant**.

Sort each scenario into the correct column, and write a one-sentence reason for each:

| Scenario | Variable or Constant? | Explanation |
|-|-|-|
| Pin number the LED is connected to | Constant | This value should realistically never be changed by the program. If it does change, this value can be edited in the program before it is compiled. |
| Current temperature sensor reading | Variable | Temperatures are generally known to change frequently. |
| Board's reference voltage (e.g. 3.3V) | Constant | This value is usually pre-defined by the product and should never change. |
| Number of times a button has been pressed | Variable | This counter should increment by 1 whenever the button in question is pressed.
| Maximum PWM speed value | Constant | This is also usually pre-defined by the product and should never change. |
| Current value from `analogRead()` | Variable | This value may change between when you check it now to when you check it in 5 minutes based on the program. |

<details>
<summary>Answer key (check after you finish)</summary>

- **Constant:** pin number, reference voltage, max PWM speed — these are fixed values that should never change once set.
- **Variable:** temperature reading, button press count, `analogRead()` value — these change while the program runs.

</details>

## Activity 4 — Start-Up + Blink 

Type every line yourself — no copy-pasting from the lesson notes.

Open this Wokwi simulation to build alongside:- https://wokwi.com/projects/470567521110438913

Work through these steps on your own, pausing to self-check after each one before moving to the next:

1. Declare `ledPin` (set to `13`) and `deviceName`.
   - *Self-check: did you use a variable for the pin instead of typing `13` directly wherever it's needed?*
2. Write `setup()` — start Serial, set pin mode, print a start-up message.
   - *Self-check: does your message print your `deviceName` variable, not typed text?*
3. Write `loop()` — blink the LED with Serial prints for `"LED: ON"` / `"LED: OFF"`.
   - *Self-check: does the Serial Monitor output match the LED blinking in real time?*
4. Upload and open the Serial Monitor to confirm it all works.

**Then, on your own:** change `deviceName` to your own name and the blink interval to 250 ms. Upload and confirm.

**Completed online @ https://wokwi.com/projects/471469797877835777**

## Activity 5 — Spot-the-Bug Worksheet 

**Format:** Solo, self-paced. For each round, read the snippet, write down what's wrong **before** revealing the answer.

**Round 1:**
```cpp
int ledPin = 13
pinMode(ledPin, OUTPUT);
```
This snippet is missing a semicolon (;) at the end of the first line.

<details><summary>Answer</summary>Missing semicolon after <code>13</code>.</details>

**Round 2:**
```cpp
void loop() {
  digitalWrite(ledPin, HIGH);
  delay(1000);
  digitalWrite(ledPin, LOW);
  delay(1000);
}

void setup() {
  Serial.begin(115200);
  pinMode(ledPin, OUTPUT);
}
```
Nothing; this snippet functions as intended.

<details><summary>Answer</summary>Not a syntax error — <code>setup()</code> always runs first regardless of where it's written in the file, so this actually still works. The real question to ask yourself: what <em>would</em> break if <code>pinMode</code> were missing entirely?</details>

**Round 3:**
```cpp
const int BUTTON_PIN = 2;
BUTTON_PIN = 3;
```
The variable <code>BUTTON_PIN</code> is originally defined with a ```constant``` value of <code>2</code> before then being attempted to be modified to a value of <code>3</code>, which will cause the program to fail to compile.

<details><summary>Answer</summary>Reassigning a <code>const</code> — this won't compile.</details>

**Round 4:**
```cpp
digitalWrite(ledPin, LOW);  // Turn LED ON
```
The comment is incorrect; the LED will be turned OFF, not ON.

<details><summary>Answer</summary>Comment doesn't match the code — <code>LOW</code> turns the LED off, not on.</details>

**Self-check:** How many did you spot correctly before looking? 

---

## Exit Ticket 

Answer on paper or in a shared doc:

1. In your own words, what's the difference between `setup()` and `loop()`?
2. Give one example of when you'd use a **constant** instead of a **variable**.


